/**
 * @hilbras/sdk — Realtime WebSocket Transport
 *
...
export class RealtimeSession {
  ...
  constructor(config: RealtimeSessionConfig) {
    this._config = { audioFormat: "pcm16", voice: "alloy", temperature: 0.8, ...config };
  }
  ...
  async connect(): Promise<void> {
    ...
    this._ws.onmessage = (event: any) => this._handleMessage(JSON.parse(event.data));
  ...
  async sendAudio(data: ArrayBuffer): Promise<void> {
    ...
    this._send({ type: "input_audio_buffer.append", audio: this._arrayBufferToBase64(data) });
  }
  ...
  async sendToolResult(callId: string, result: unknown): Promise<void> {
    if (!this._connected) throw new Error("Not connected");
    this._send({ type: "conversation.item.create", item: { type: "function_call_output", call_id: callId, output: typeof result === "string" ? result : JSON.stringify(result) } });
    this._send({ type: "response.create" });
  }
  ...
  private _getBaseUrl(): string {
    switch (this._config.provider) {
      case "openai": return "wss://api.openai.com/v1/realtime";
      ...
      case "xai": return "wss://api.x.ai/v1/realtime";
      default: throw new Error(`Unsupported provider: ${this._config.provider}`);
    }
  }

  private _send(data: Record<string, unknown>): void {
    if (this._ws?.readyState === 1) this._ws.send(JSON.stringify(data));
  }
  ...
  private _handleMessage(data: Record<string, unknown>): void {
    switch (data.type) {
      case "session.created":
        ...
        break;
      case "session.deleted": this._connected = false; break;
      case "response.text.delta": this._emit({ type: "text", text: (data.delta as string) ?? "", done: false }); break;
      case "response.text.done": this._emit({ type: "text", text: "", done: true }); break;
      case "response.audio.delta": this._emit({ type: "audio", data: this._base64ToArrayBuffer((data.delta as string) ?? ""), done: false }); break;
      case "response.audio.done": this._emit({ type: "audio", data: new ArrayBuffer(0), done: true }); break;
      case "response.function_call_arguments.done":
        this._emit({ type: "tool_call", name: (data.name as string) ?? "", arguments: JSON.parse((data.arguments as string) ?? "{}"), callId: (data.call_id as string) ?? "" });
        break;
      case "error": this._emit({ type: "error", error: new Error((data.error as any)?.message ?? "Unknown error") }); break;
    }
  }

  private _emit(event: RealtimeEvent): void {
    for (const handler of this._handlers.get(event.type) ?? []) handler(event);
    for (const handler of this._handlers.get("*") ?? []) handler(event);
  }
