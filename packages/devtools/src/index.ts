/**
 * @hilbras/devtools — Visual Debugging Interface
 *
 ...
 * @example
 * ```ts
 * import { DevTools } from "@hilbras/devtools";
 *
 * const devtools = new DevTools({ logLevel: "detailed" });
 *
 * // Hook into client events
 * client.hooks.on("request.start", (event) => devtools.onRequestStart(event));
 * client.hooks.on("request.completed", (event) => devtools.onRequestComplete(event));
 *
 * // Or use the built-in HTML panel
 * devtools.mount(document.getElementById("devtools"));
 * ```
 */

export interface DevToolsConfig {
  /** Log level: "minimal" | "detailed" | "verbose" */
...
export interface LogEntry {
  ...
  provider?: string;
  model?: string;
  /** Input messages (truncated in minimal mode) */
  input?: string;
  /** Output text */
  output?: string;
  /** Token usage */
...
export class DevTools {
  ...
  onRequestStart(event: { requestId: string; provider?: string; model?: string; input?: string }): void {
    ...
    if (this._config.logLevel === "verbose") {
      this._addToHistory({
        ...
        model: event.model,
        input: this._truncate(event.input, 200),
      });
    }
  }

  /**
   * Called when the first chunk arrives (for TTFT).
   */
  onRequestChunk(event: { requestId: string }): void {
    const active = this._activeRequests.get(event.requestId);
    if (active && !active.firstChunk) {
      active.firstChunk = performance.now();
    }
  }
  ...
  onRequestComplete(event: {
    ...
    const tokensPerSecond = event.usage?.output && durationMs > 0
      ? (event.usage.output / durationMs) * 1000
      : undefined;

    const entry: LogEntry = {
      id: event.requestId,
      timestamp: Date.now(),
      type: event.error ? "error" : "response",
      provider: event.provider,
      model: event.model,
      input: this._truncate(event.input, this._config.logLevel === "minimal" ? 50 : 200),
      output: this._truncate(event.output, 500),
      usage: event.usage,
      cost: event.cost,
  ...
  /**
   * Get metrics for all recorded requests.
   */
  getMetrics(): RequestMetrics {
    ...
    for (const entry of entries) {
      if (entry.provider) {
        ...
        byProvider[entry.provider].tokens += entry.usage?.total ?? 0;
      ...
      if (entry.model) {
        ...
        byModel[entry.model].tokens += entry.usage?.total ?? 0;
    ...
    return {
      ...
      failedRequests: entries.length - successful.length,
      totalTokens,
      totalCost,
  ...
  private _printEntry(entry: LogEntry): void {
    ...
    if (entry.output && this._config.logLevel === "verbose") {
      console.log(`  Output: ${this._truncate(entry.output, 200)}`);
    }
    if (entry.toolCalls?.length) {
      console.log(`  Tools: ${entry.toolCalls.map((t) => t.name).join(", ")}`);
    }
    if (entry.error) {
