...
describe("Vue parseUIStream", () => {
  ...
  it("parses SSE stream into protocol messages", async () => {
    ...
    expect(results[1]).toEqual({ type: "text_delta", text: "Bonjour" });
    expect(results[2]).toEqual({ type: "text_delta", text: " le monde" });
...
describe("Vue useObject incremental parsing", () => {
  function encodeSSE(messages: UIProtocolMessage[]): Uint8Array {
    const encoder = new TextEncoder();
    const parts = messages.map((m) => `data: ${JSON.stringify(m)}\n\n`);
    return encoder.encode(parts.join("") + "data: [DONE]\n\n");
  ...
  it("incrementally parses tool_call deltas into a partial object", async () => {
    ...
    const messages: UIProtocolMessage[] = [
      ...
      { type: "tool_call_delta", id: "c1", args: ' "Jean",' },
      { type: "tool_call_delta", id: "c1", args: ' "city":' },
      { type: "tool_call_delta", id: "c1", args: ' "Paris"}' },
    ...
    expect(obj).toEqual({ name: "Jean", city: "Paris" });
  });
});

// ─── Composition Function Signatures ────────────────────────────────────────

describe("Vue composition function exports", () => {
  it("useChat is exported and is a function", async () => {
    const mod = await import("../src/frameworks/vue/use-chat.js");
    expect(typeof mod.useChat).toBe("function");
  });

  it("useCompletion is exported and is a function", async () => {
    const mod = await import("../src/frameworks/vue/use-completion.js");
    expect(typeof mod.useCompletion).toBe("function");
  });

  it("useObject is exported and is a function", async () => {
    const mod = await import("../src/frameworks/vue/use-object.js");
